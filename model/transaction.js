...
class transaction {
  ...
  class TransactionManager {
    ...
    static getTransaction = async (userUuid) => {
      const pgClient = await newClient ();
      const queryRes = await pgClient.query ( 
        "SELECT transactions.transaction_id, transactions.personal_id,transactions.business_id, transactions.total_amount, transactions.date_time, transactions.went_trough,paypulp_customers.user_uuid, paypulp_customers.first_name, business_accounts.business_name, products.product_name, payment_methods.card_number, payment_methods.is_preferred FROM transactions INNER JOIN personal_accounts ON transactions.personal_id = personal_accounts.personal_id INNER JOIN business_accounts ON transactions.business_id = business_accounts.business_id INNER JOIN products ON transactions.product_uuid = products.product_uuid  INNER JOIN payment_methods ON transactions.pay_method_uuid = payment_methods.pay_method_uuid INNER JOIN paypulp_customers ON business_accounts.customer_id = paypulp_customers.customer_id OR personal_accounts.customer_id = paypulp_customers.customer_id WHERE user_uuid = ($1) ORDER BY date_time DESC;", [userUuid]
      );
